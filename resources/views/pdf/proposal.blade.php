...
<head>
    <meta charset="utf-8">
    <title>Proposta {{ $record->proposal_number }}</title>
    <style>
...
<body>
    ...
    <table class="meta">
        ...
        <tr>
            ...
            <td>{{ $record->proposal_date?->format('d/m/Y') }}</td>
        </tr>
        <tr>
            <td class="label">Validade</td>
            <td>{{ $record->valid_until ? $record->valid_until->format('d/m/Y') : '—' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>{{ $record->proposal_status?->label() ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">Valor</td>
            <td>
                @if($record->proposal_amount !== null)
                    {{ number_format((float) $record->proposal_amount, 2, ',', ' ') }} €
                @else
                    —
                @endif
            </td>
